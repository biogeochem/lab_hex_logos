# carbulate hex sticker
# https://github.com/paukes/carbulate

library(carbulate)
library(tidyverse)
library(hexSticker)
library(showtext)
font_add_google("Baloo") # https://fonts.google.com

# make data for the Bjerrum plot

df <- data.frame(DIC_mgC.L = 1, pH = seq(0.1,14,0.1), Temp_C = 20, 'pressure_kPa' = 101.325)

df <- carbulate(df, 'DIC_mgC.L', 'pH', 'Temp_C', 'pressure_kPa')

df_long <- df %>% select(pH, contains("uM"), -calc_carb_alk_uM) %>% 
  pivot_longer(cols = contains("uM"), 
               names_to = "species", values_to = "concentrations")

# make Bjerrum plot

x <- ggplot(df_long, aes(x = pH, y = concentrations, colour = species)) +
  geom_line(size = 1, show = FALSE) +
  scale_colour_viridis_d() +
  scale_x_continuous(limits = c(0, 14))

x <- x +
  theme_void() +
  theme_transparent() +
  theme(legend.position="none")

# make sticker

sticker(x,
        package = "carbulate",
        h_fill = "lightblue", h_color = "orange", h_size = 1.5,
        p_color = "#000000", p_y = 1.5, p_size = 14,  p_family = "Baloo",
        s_width = 1, s_height = 0.8, s_x = 1, s_y = 0.9,
        url = "github.com/paukes/carbulate", u_size = 5,
        filename = "output/carbulate.png")
